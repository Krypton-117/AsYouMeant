...
import { reviewGate, type GateConflict, type GateDimension } from "../../conformance/reviewer.js";
import { codexWriteTargets, type CodexHookInput, type CodexHookOutput, type CodexPermitStore, type CodexRuntimeContract } from "./adapter.js";

const axes: GateDimension[] = ["intent", "permission", "technical", "internal"];
interface IntentSummary { problem: string; outcome: string; acceptanceCriteria: string[] }
interface Draft extends CodexRuntimeContract { intent: IntentSummary }
interface Preparation {
  authorSession: string;
  stage: "draft" | "frozen" | "reviewed" | "rejected";
  contract: Draft | null;
  reviewerSession?: string;
}
interface Options { stateRoot: string; contractPath: string; store: CodexPermitStore }
interface Result { handled: boolean; output?: CodexHookOutput; contract?: CodexRuntimeContract }
...
function assertPlain(path: string): void {
  ...
  for (let current = resolve(path); ; current = dirname(current)) {
    try {
      if (lstatSync(current).isSymbolicLink()) throw new Error("Preparation paths must not contain symbolic links or junctions.");
    } catch (error) {
...
function message(input: CodexHookInput, text: string, denied = false): Result {
  return { handled: true, output: { hookSpecificOutput: input.hook_event_name === "PreToolUse"
    ? { hookEventName: "PreToolUse", ...(denied ? { permissionDecision: "deny" as const, permissionDecisionReason: text } : { additionalContext: text }) }
    : { hookEventName: "UserPromptSubmit", additionalContext: text } } };
}
function validatedDraft(value: unknown, candidate: string): Draft {
  const draft = value as Draft;
  if (!draft || draft.candidateVersion !== candidate || candidate === "unreviewed") throw new Error("The command candidate must match the completed draft candidateVersion.");
  ...
  if (!nonempty(draft.activeWorkItemId) || draft.activeWorkItemId === "pre-loop" || draft.actionBasis?.kind !== "requested"
    || !draft.actionBasis.requirementIds.length || !draft.actionBasis.requirementIds.every(nonempty)) throw new Error("Name the requested work item and requirement IDs.");
...
export function handleCodexPreparation(input: CodexHookInput, options: Options): Result {
  ...
  if (!command && !existsSync(statePath)) return { handled: false };
  if (!command && !starting && options.store.mode(session) === "ordinary") return { handled: false };
  ...
  try {
    if (!session) throw new Error("A native session identity is required.");
    assertPlain(directory);
    assertPlain(statePath);
    for (const file of [draftPath, options.contractPath, join(directory, "review-request.md"), join(directory, "review.json")]) assertPlain(file);
    let state = existsSync(statePath) ? readJson(statePath) as Preparation : null;
    if (command) {
      if (resolve(options.contractPath) !== join(directory, "contract.json")) throw new Error("Native preparation uses the current project's .asyoumeant/contract.json; remove a custom contract-path override for this route.");
      if (prompt === "$pre-loop-governor prepare") {
        if (state?.stage === "draft" && state.authorSession !== session) throw new Error("Another session owns this draft; return to that session to prepare it.");
        options.store.setMode(session, "aym");
        if (!existsSync(draftPath)) writeJson(draftPath, {
      ...
      if (!state) throw new Error("Start with $pre-loop-governor prepare in the author session.");
      const freeze = /^\$pre-loop-governor freeze candidate=([A-Za-z0-9][A-Za-z0-9._-]{0,79})$/.exec(prompt);
      if (freeze) {
        if (state.authorSession !== session || state.stage !== "draft") throw new Error("Only the author session may freeze an active draft.");
      ...
      if (review) {
        if (session === state.authorSession) throw new Error("Independent review must be recorded in a different native session from the author.");
        if (state.stage !== "frozen" || !state.contract || state.contract.candidateVersion !== review[1] || state.contract.projectionIdentity !== review[2]) throw new Error("Review must bind the current frozen candidate AND projection.");
        if (digest(readJson(options.contractPath)) !== digest(state.contract)) throw new Error("The frozen contract changed; prepare and freeze it again before review.");
        const findings = JSON.parse(review[3]!) as Record<GateDimension, { pass: boolean; evidence: string }>;
        if (!findings || Object.keys(findings).length !== axes.length || !axes.every(axis => typeof findings[axis]?.pass === "boolean" && nonempty(findings[axis]?.evidence))) throw new Error("Independent review requires four explicit pass/fail findings with evidence.");
        const conflicts: GateConflict[] = axes.filter(axis => !findings[axis].pass).map(axis => ({
    ...
    if (input.hook_event_name === "PreToolUse") {
      ...
      if (write) for (const target of targets) assertPlain(resolve(workspace, target));
      if (write && targets.some(path => path.split("/").some(part => [".asyoumeant", ".git", ".codex", ".agents"].includes(part)))) {
        ...
        if (draftOnly) return message(input, "AYM pre-loop draft write only; this does not authorize implementation or review.");
    ...
    if (state.contract && digest(readJson(options.contractPath)) !== digest(state.contract)) throw new Error("Frozen authority changed. No old permit applies; prepare, freeze and review again.");
