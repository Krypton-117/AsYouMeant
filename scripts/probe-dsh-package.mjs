...
const options = { dshBin, repositoryRoot: resolve("."), packageRoot: "", dshHome, allowedQaRoot: qaRoot, contractPath: join(dshHome, "absent-contract.json"), commandTimeoutMs: 60000 };
let installed = false;
try {
  ...
  installed = true;
  const profilePath = join(dshHome, "profiles", "headless", "package.json");
  ...
  const installedManifest = JSON.parse(readFileSync(join(dshHome, "profiles", "headless", "node_modules", packed.name, "package.json"), "utf8"));
  if (installedManifest.version !== packed.version) throw new Error("Installed package version differs from tarball.");
  for (const file of packed.files) {
    const installedFile = readFileSync(join(dshHome, "profiles", "headless", "node_modules", packed.name, file));
  ...
  console.log(JSON.stringify({ status: "PASS", hostVersion: DSH_VERSION, package: `${packed.name}@${packed.version}`, install: fromRegistry ? "registry" : "tarball", contents: "source-equal", composition: "verified", uninstall: "verified", authenticatedModelRun: "not-requested", dailyProfileChanged: false }, null, 2));
} finally {
