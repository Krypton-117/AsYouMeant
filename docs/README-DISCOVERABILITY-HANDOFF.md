...
2026-09-08 开发补充（未发布）：根据 [细粒度审计](README-AUDIT-AND-REVISION-PLAN.md)，先补 Codex 的有限准备链，再修正 README 承诺。详见 [首个任务指南](CODEX-FIRST-TASK.md)。Hook 测试与真实模型流程必须分开记录；此次真实 CLI 探测 25 秒超时且没有创建草稿，原因未定位，不可写成用户闭环通过。以下旧测试数字是历史记录。
